 ...
 */
class SearchableGrailsPlugin extends Plugin {
...
'''
    ...
    Closure doWithSpring() {
        { ->
            // Configuration
            def config = getConfiguration(grailsApplication.parentContext, grailsApplication)

            // Compass
            LOG.debug("Defining Compass and Compass::GPS beans")
            def defaultConnection = getDefaultCompassConnection(grailsApplication)
...
//            lifecycleInjector = lifecycleInjector
    ...
    private getDefaultCompassConnection(GrailsApplication application) {
